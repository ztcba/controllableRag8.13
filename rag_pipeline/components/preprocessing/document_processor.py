...
def main():
    ...
    # 处理单个文档
    input_file = Path("data/test_set/databases/chunked_reports/guide.json")  # 替换为实际PDF路径
    output_dir = Path("output/processed_documents")
    
    if input_file.suffix.lower() == '.pdf':
        result = processor.process_single_document(input_file, output_dir)
        print(f"处理结果: {result}")
    else:
        # 如果是目录，批量处理
        input_dir = Path("input_pdfs")  # 替换为实际目录
